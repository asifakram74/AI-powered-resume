...
export const exportToDOCX = async (htmlContent: string, filename: string = 'cv.docx') => {
  try {
    console.log('Making request to:', 'https://render-kweq.onrender.com/om/api/export-docx');
    ...
    const response = await fetch('https://render-kweq.onrender.com/om/api/export-docx', {
      method: 'POST',
      headers: {
