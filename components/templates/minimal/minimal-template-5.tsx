...
export function MinimalTemplate5({ data, isPreview = false, profileImage }: MinimalTemplate5Props) {
  ...
  const SectionTitle = ({ title }: { title: string }) => (
    <h2 className="text-xl font-bold text-slate-800 mb-6 pb-3 border-b-2 border-indigo-600">{title}</h2>
  )
    ...
    if (remainingContent.length > 0) {
      items.push(
        <TwoColumnLayout
          key="page-2"
          sidebar={
            <div className="w-60 bg-indigo-950 text-white p-10">
              <div className="text-indigo-950 text-sm italic">
