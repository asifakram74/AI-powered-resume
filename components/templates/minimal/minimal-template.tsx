...
export function MinimalTemplate({ data, isPreview = false }: MinimalTemplateProps) {
  ...
  const SectionTitle = ({ title }: { title: string }) => (
    <h2 className="text-xl font-bold text-slate-800 mb-6 pb-3 border-b-2 border-amber-600">{title}</h2>
  )
  ...
  const blocks = useMemo(() => {
    ...
    if (data.experience.length > 0 || data.education.length > 0 || data.projects.length > 0 || data.certifications.length > 0) {
      // Calculate if we need more pages for education, projects, certifications
      ...
      // Add remaining content to additional pages
      if (remainingContent.length > 0) {
        items.push(
          <TwoColumnLayout
            key="page-2"
            sidebar={
              <div className="w-60 bg-slate-900 text-white p-10">
                {/* Empty sidebar for continuation pages */}
                <div className="text-slate-900 text-sm italic">
                  Continued from previous page...
                </div>
              </div>
            }
            main={<div className="space-y-2">{remainingContent}</div>}
          />
        );
      }
    }
