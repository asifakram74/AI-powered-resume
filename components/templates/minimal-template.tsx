  ...
  return (
    <div className="max-w-3xl mx-auto bg-white p-16 min-h-screen">
      ...
      <div className="mb-16">
        ...
        <h2 className="text-2xl font-light text-gray-600 mb-6">{data.personalInfo.jobTitle}</h2>
      ...
      {/* Summary */}
      <div className="mb-16">
        <p className="text-gray-700 text-lg leading-relaxed font-light">{data.personalInfo.summary}</p>
      </div>
          ...
          {data.education.map((edu) => (
            <div key={edu.id}>
              <div className="flex justify-between items-baseline mb-1">
                ...
                {edu.graduationDate && <span className="text-gray-500 text-sm">{formatDate(edu.graduationDate)}</span>}
              </div>
              ...
              {edu.additionalInfo && <p className="text-gray-600 text-sm mt-2">{edu.additionalInfo}</p>}
            </div>
          ))}
        </div>
      </div>
                ...
                {(project.liveDemoLink || project.githubLink) && (
                  <div className="text-gray-500 text-sm space-x-4">
                    {project.liveDemoLink && (
                      <a href={project.liveDemoLink} className="hover:text-gray-700">
                        Live Demo
                      </a>
                    )}
                    {project.githubLink && (
                      <a href={project.githubLink} className="hover:text-gray-700">
                        GitHub
                      </a>
                    )}
                  </div>
              ...
              <div key={cert.id} className="flex justify-between items-baseline">
                ...
                <div className="text-right">
                  ...
                  {cert.verificationLink && (
                    <div>
                      <a href={cert.verificationLink} className="text-gray-500 text-sm hover:text-gray-700">
                        Verify
                      </a>
                    </div>
                  )}
